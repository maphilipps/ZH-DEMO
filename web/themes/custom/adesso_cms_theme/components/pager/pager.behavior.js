...
(function (Drupal, once) {
  ...
  function initializePager(pagerElement) {
    ...
    const contentContainer = document.querySelector(pagerElement.getAttribute('data-content-target')) ||
                           pagerElement.closest('.content-with-pager')?.querySelector('.pager-content') ||
    ...
    initializePagerLinks(pagerLinks, pagerElement, contentContainer, enableAjax, updateHistory);

    // Initialize keyboard navigation
    initializeKeyboardNavigation(pagerLinks, pagerElement);

    // Initialize infinite scroll if enabled
    const infiniteScroll = pagerElement.getAttribute('data-infinite-scroll') === 'true';
    if (infiniteScroll) {
    ...
    console.log('[adesso-pager] Pager initialized with', pagerLinks.length, 'links');
  }
  ...
  function initializePagerLinks(pagerLinks, pagerElement, contentContainer, enableAjax, updateHistory) {
    pagerLinks.forEach(function (link) {
      ...
      const pageNumber = link.getAttribute('data-page') || extractPageFromUrl(link.href);
      if (pageNumber) {
   ...
   * @return {void}
   */
  function loadPageContent(link, pagerElement, contentContainer, updateHistory) {
    const url = link.href;
    const pageNumber = link.getAttribute('data-page') || extractPageFromUrl(url);
  ...
  function initializeKeyboardNavigation(pagerLinks, pagerElement) {
    const activeLinks = Array.from(pagerLinks).filter(link => 
  ...
  function initializeInfiniteScroll(pagerElement, contentContainer) {
    if (!contentContainer) {
      console.warn('[adesso-pager] Content container not found for infinite scroll');
      return;
    }

    const nextLink = pagerElement.querySelector('.pager-next, [data-page-next]');
      ...
      entries.forEach(function (entry) {
        if (entry.isIntersecting && !pagerElement.classList.contains('loading')) {
          ...
          if (currentNextLink && !currentNextLink.classList.contains('disabled')) {
            loadNextPageInfinite(currentNextLink, pagerElement, contentContainer);
          }
        }
      });
    }, {
  ...
  function initializeHistoryManagement(pagerElement) {
    window.addEventListener('popstate', function (e) {
      if (e.state && e.state.page) {
        ...
        const pageLink = pagerElement.querySelector(`[data-page="${currentPage}"], [href*="page=${currentPage}"]`);
  ...
  function scrollToContent(contentContainer) {
    ...
    const targetPosition = contentContainer.getBoundingClientRect().top + window.pageYOffset - offset;
  ...
  function showErrorMessage(pagerElement, message) {
    ...
    if (!errorDiv) {
      ...
      errorDiv.className = 'pager-error text-red-600 text-sm mt-2 p-2 bg-red-50 rounded';
