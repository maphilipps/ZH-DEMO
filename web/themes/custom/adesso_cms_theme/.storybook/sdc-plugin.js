 ...
 */
function getDependencyImports(storiesPath, componentDirectory, namespaces = ['adesso_cms_theme']) {
  ...
  const scanDataFile = (dataPath, currentDir) => {
    ...
    // Add twig imports
    Array.from(dataContent.matchAll(/import.*from\s+'([^']*\.twig)'/g)).forEach((match) => twigFiles.add(path.resolve(currentDir, match[1])));

    // Scan nested data files
    Array.from(dataContent.matchAll(/import.*from\s+'([^']*\.stories\.data)'/g)).forEach((match) => {
  ...
  const scanTwigFile = (twigPath) => {
    ...
    Array.from(twigContent.matchAll(includeRegex)).forEach((match) => {
  ...
  if (fs.existsSync(storiesPath)) {
    ...
    Array.from(storiesContent.matchAll(/import.*from\s+'([^']*\.twig)'/g)).forEach((match) => twigFiles.add(path.resolve(dir, match[1])));

    // Scan data files
    Array.from(storiesContent.matchAll(/import.*from\s+'([^']*\.stories\.data)'/g)).forEach((match) => scanDataFile(path.resolve(dir, `${match[1]}.js`), dir));
  }
...
export default (options = {}) => ({
  ...
  transform: (code, id) => {
    ...
    const namespaces = options.namespaces || ['adesso_cms_theme'];
