 ...
 */
function adesso_cms_installer_install_tasks(): array {
  ...
  \Drupal::service('class_loader')
    ->addPsr4('Drupal\\adesso_cms_installer\\', __DIR__ . '/src');
 ...
 */
function adesso_cms_installer_install_tasks_alter(array &$tasks, array $install_state): void {
  ...
  $insert_before('install_profile_modules', [
    'install_install_profile' => [
      'function' => 'adesso_cms_installer_install_myself',
    ],
  ...
  $tasks['install_profile_modules']['function'] = 'adesso_cms_installer_apply_recipes';
}
 ...
 */
function adesso_cms_installer_install_myself(array &$install_state): void {
 ...
 */
function adesso_cms_installer_form_install_settings_form_alter(array &$form): void {
 ...
 */
function adesso_cms_installer_form_install_configure_form_alter(array &$form, FormStateInterface $form_state): void {
  ...
  $form['#submit'][] = 'adesso_cms_installer_update_site_mail';
  ...
  $form['admin_account']['account']['pass'] = [
    ...
    '#weight' => 20,
    '#value_callback' => '_adesso_cms_installer_password_value',
  ];
 ...
 */
function adesso_cms_installer_update_site_mail(array &$form, FormStateInterface $form_state): void {
...
function _adesso_cms_installer_password_value(&$element, $input, FormStateInterface $form_state): mixed {
 ...
 */
function adesso_cms_installer_library_info_alter(array &$libraries, string $extension): void {
  global $install_state;
 ...
 */
function adesso_cms_installer_uninstall_myself(): void {
 ...
 */
function adesso_cms_installer_preprocess_install_page(array &$variables): void {
  // Don't show the task list or the version of Drupal.
  unset($variables['page']['sidebar_first'], $variables['site_version']);

  global $install_state;
  $images_path = $install_state['profiles']['adesso_cms_installer']->getPath() . '/images';
