...
function renderAccordion(props = {}) {
  ...
  const itemsHtml = accordion_items.map((item, index) => {
...
describe('Accordion Component', () => {
  ...
  describe('Accordion Items', () => {
    it('should render item title and content', () => {
      const accordionHtml = renderAccordion({
        accordion_items: [
          { title: 'How does this work?', content: 'This is how it works...', expanded: false }
        ]
      });
      container.innerHTML = accordionHtml;
    ...
    it('should handle expanded state correctly', () => {
      const accordionHtml = renderAccordion({
        accordion_items: [
          { title: 'Expanded Item', content: 'Visible content', expanded: true },
          { title: 'Collapsed Item', content: 'Hidden content', expanded: false }
        ]
      });
      container.innerHTML = accordionHtml;
  ...
  describe('Multiple Items', () => {
    ...
    it('should generate unique IDs for each item', () => {
      ...
      expect(triggers[0].getAttribute('aria-controls')).toBe('accordion-content-0');
      expect(triggers[1].getAttribute('aria-controls')).toBe('accordion-content-1');
